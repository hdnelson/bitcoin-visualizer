...
public class BlockType
{
	...
	/**
	 *  
	 * @return Full hash of this block. Sometimes this is expressed without the leading
	 * zeroes.
	 */
	...
	/**
	 * 
	 * @param hash Full hash of this block. Sometimes this is expressed without the leading
	 * zeroes.
	 */
	...
	public void parseBlock(String blockString) throws JSONException
	{
		...
		// Components of a transaction.  Note: some are shared with the block: version, hash, lock time
		final String TX = "tx";
		...
		this.setVersion(block.getString(VER));
		this.setPrev_block(block.getString(PREV_BLOCK));
		this.setMrkl_root(block.getString(MRKL_ROOT));
		this.setTime(new Date(Long.parseLong(block.getString(TIME)) * 1000));		
		...
		for (int i = 0; i < transactionsArray.length(); i++)
		{
			...
			// Next we are going to get all the incoming transaction address information
			...
			for (int j = 0; j < incomingTransactionsArray.length(); j++)
			{
				...
				// Sometimes transactions dont have a coinbase and sometimes they dont have a scriptSig, so we will wrap this in a try so an exception isn't thrown
				try
				{
			...
			// Next we are going to get all the outgoing transaction address information
